		...
		for (int i = 0; i < room.getWidth(); i++) {
			for (int j = 0; j < room.getHeight(); j++) {
				...
				if (hex.matches(TileHex.RED_WALL)) {
					tiles[i][j] = new Tile(new Vector2(i * Tile.size, j * Tile.size), TileType.RED_WALL);
				} else if (hex.matches(TileHex.WOOD_FLOOR)) {
					tiles[i][j] = new Tile(new Vector2(i * Tile.size, j * Tile.size), TileType.WOOD_FLOOR);
				} else if (hex.matches(TileHex.STONE_FLOOR)) {
					tiles[i][j] = new Tile(new Vector2(i * Tile.size, j * Tile.size), TileType.STONE_FLOOR);
				}
			}
		}
	}
